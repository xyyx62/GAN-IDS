...
PDF_NAME = ''
# 开启蜜罐docker
#@app.route('/starthoneypot/',method=['POST','GET'])
def starthoneypot():
    #filepath = app.config['UPLOAD_FOLDER']
    '''
    基础数据解析
    ...
    #NAME = capture()
    PCAP_NAME = capture()
    #PCAP_NAME = PCAP_NAME.rstrip()
    #PCAP_NAME = str(PCAP_NAME)
    #PCAPS = rdpcap(os.path.join(filepath,PCAP_NAME))
    #A = '2019-12-12 06:12:44.065673.pcap'
    #path = str(A)
    PCAPS = rdpcap(os.path.join(filepath,PCAP_NAME))
    print(os.path.join(filepath,PCAP_NAME))
    #os.system('rm -rf ' + filepath + '*')
    filter_type = request.form.get('filter_type', type=str, default=None)
    value = request.form.get('value', type=str, default=None)
    # 如果有选择，通过选择来获取值
    if filter_type and value:
        ...
        pcaps = get_all_pcap(PCAPS, PD)
    #return render_template('./dataanalyzer/basedata.html', pcaps=pcaps)
    flash('开启蜜罐服务成功！可以抓取数据包啦')
    return render_template('./dataanalyzer/capture.html', pcaps=pcaps)
        #flash("请先上传要分析的数据包!")
        #return redirect(url_for('upload'))
# 详细数据

@app.route('/datacapture/', methods=['POST', 'GET'])
def datacapture():
    filepath = app.config['UPLOAD_FOLDER']
    '''
    基础数据解析
...
def flowanalyzer():
    if PCAPS == None:
        ...
        print(max(time_flow_dict.values()))
        print(min(time_flow_dict.values()))
